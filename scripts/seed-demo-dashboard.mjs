...
async function main() {
  console.log('🌱 Seeding demo data for Durable Observability Dashboard...');
  ...
  // Clean old demo runs for tenant to ensure fresh timestamps
  ...
  await prisma.workflowRun.create({
    data: {
      id: 'aaaaaaaa-4444-4000-8000-000000000004',
      tenantId,
      ...
      cancelledAt: new Date(now - 280_000),
      createdAt: new Date(now - 305_000),
      input: { email: 'canceled.user@acme.com' },
    },
  });

  // Additional Completed runs for pagination
  for (let i = 5; i <= 10; i++) {
    await prisma.workflowRun.create({
      data: {
  ...
  console.log(`Flagship Run: ${run1Id} (COMPLETED with Concurrent steps)`);
  console.log(`Failed Run:   ${run2Id} (FAILED, ready to test Retry)`);
  console.log(`Running Run:  ${run3Id} (RUNNING, ready to test Cancel)`);
  console.log('----------------------------------------------------');
}
