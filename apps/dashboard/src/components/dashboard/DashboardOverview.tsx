    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard
            title="Total Runs"
            value={isMetricsLoading ? '—' : (metrics?.totalRuns ?? 0)}
            icon={<BarChart3 className="h-5 w-5" />}
            tone="default"
          />
          <MetricCard
            title="Active Runs"
            value={isMetricsLoading ? '—' : (metrics?.activeRuns ?? 0)}
            icon={<Activity className="h-5 w-5" />}
            tone="blue"
          />
          <MetricCard
            title="Completed Runs"
            value={isMetricsLoading ? '—' : (metrics?.completedRuns ?? 0)}
            icon={<CheckCircle2 className="h-5 w-5" />}
            tone="green"
          />
          <MetricCard
            title="Failed Runs"
            value={isMetricsLoading ? '—' : (metrics?.failedRuns ?? 0)}
